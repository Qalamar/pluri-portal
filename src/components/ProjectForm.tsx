...
const ProjectForm: React.FC = observer(() => {
  ...
  const onSubmit = () => {
    let docu = Project.document.getAll("document")[
      Project.document.getAll("document").length - 1
    ];
          ...
          <Controller
            as={
              <IonSelect
                ...
                value={selectpromo}
                onIonChange={(e) => setSelectpromo(e.detail.value)}
              >
                {promos.map((promo: any, i) => {
                  return (
                    <IonSelectOption value={promo.id}>
                      {promo.year}
